function Connect-MAT {
    ...
    # This prevents token/credential bleed-over when switching tenants.
    Disconnect-ExchangeOnline -Confirm:$false -ErrorAction SilentlyContinue
    Disconnect-MgGraph -ErrorAction SilentlyContinue
    # No -ListAvailable here: we only call Disconnect-AzAccount if Az.Accounts is
    # currently LOADED in this session (i.e. was used before). -ListAvailable would
    # check disk installation regardless of whether it is active — wrong intent.
    ...
    # Wipe ALL previous session data so the UI header returns to DISCONNECTED immediately.
    # If the new connection fails partway through, Initialize-MATState is called again in
    # the catch block — so the header never shows stale data from the previous tenant.
    Initialize-MATState
    ...
    try {
        ...
        # Required scopes — each is load-bearing:
        #   Directory.Read.All           tenant/org info, SKUs, user object reads
        #   AuditLog.Read.All            UAL probe queries
        #   User.Read.All                group and role membership lookups
        #   RoleManagement.Read.Directory  active role assignments + PIM eligibility
        #   Policy.Read.All              CA policies + Security Defaults
        #                                NOT covered by Directory.Read.All (common mistake)
        #   Reports.Read.All             Copilot usage summary endpoint
        ...
        # Step 1: Try with explicit $select — smaller response, works on most tenants.
        $tenantInfo = Get-MgOrganization -Property DisplayName, Id -ErrorAction Stop |
                      Select-Object -First 1

        # Step 2: Some SDK versions and tenant types (GCC, EDU, partner tenants) return
        # a null/empty DisplayName even on a successful query with -Property.
        # Retry without -Property to let the SDK return the full default field set.
        ...
        # Fallback chain: real display name → short GUID label → generic string.
        # Every branch produces a non-empty string so the UI header is never blank.
        ...
        # Pre-warm SKU cache so all three audit modes share one Graph round-trip.
        ...
        # Exchange Online — pinned to the same account to prevent credential bleed-over.
        ...
        try {
            ...
            # Hierarchy — most permissive role listed first so the display
            # reflects the highest privilege actually active in this session.
            ...
            # PIM NOTE: Get-MgUserMemberOf only returns ACTIVE (activated) role memberships.
            # If your role is PIM-eligible but not yet activated for this session, it will
            # not appear here and MAT will show "Standard User" or a lower role.
            # Activate your PIM assignment in the Entra ID portal before connecting.
        ...
        # -ListAvailable IS correct here: we need to know if Az.Accounts is installed
        # on disk before attempting to use it. The disconnect guard at the top of this
        # function deliberately omits -ListAvailable because it only wants to disconnect
        # if the module is already active in the current session.
        ...
        # Explicitly disconnect any service that may have partially authenticated
        # before the failure — prevents a zombie Graph session from persisting while
        # the UI shows DISCONNECTED state.
        Disconnect-MgGraph -ErrorAction SilentlyContinue
        Disconnect-ExchangeOnline -Confirm:$false -ErrorAction SilentlyContinue

        # Full state reset — UI returns cleanly to DISCONNECTED
        Initialize-MATState
