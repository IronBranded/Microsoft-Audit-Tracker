...
#   - Hidden rows use height:0 so filtered rows collapse instead of leaving gaps
# ================================================================================
...
function New-MATHtmlReport {
    ...
    # Main table rows (from in-memory data — no re-read)
    ...
    # Copilot sub-section rows — filtered from in-memory data (zero extra file reads)
    ...
    # ── PER-SECTION ISSUE COUNTS for badges ───────────────────────────────────
    ...
    # Badge HTML helper — returns "" when section is clean.
    # All computed as PS variables to avoid nested-quote issues in the here-string.
...
th{background:rgba(88,166,255,.07);color:var(--pr);text-align:left;padding:12px 15px;
   font-size:11px;text-transform:uppercase;font-weight:600;border-bottom:2px solid var(--bd);
   position:sticky;top:52px;z-index:10;white-space:nowrap}
.section.cps th{background:rgba(163,113,247,.07);color:var(--cp)}
...
/* Zebra — non-highlighted rows only */
...
/* Filtered-out rows collapse cleanly without leaving blank space */
...
@media print{
  ...
  th{position:static;background:#eef2ff;color:#333;border-bottom:2px solid #bbb}
  ...
  /* CONFIDENTIAL diagonal watermark — print/PDF only */
