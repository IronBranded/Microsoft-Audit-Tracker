function Invoke-AuditorMode {
    ...
    #>
    param(
        # Bug Fix #2 / Improvement: Silent switch suppresses Pause for SuperAuditor calls.
        [switch]$Silent
    )
    ...
    try {
        # Note: Get-AzDiagnosticSetting requires the Az.Monitor module (optional install).
        # If Az.Monitor is missing, this catch block fires and the row is marked Manual Check.
        ...
        Write-Host "[!] Info: Entra ID diagnostic check requires Az.Monitor (Install-Module Az.Monitor)" -ForegroundColor Yellow
        Add-AuditRow "Identity" "Entra ID Diagnostic" "Manual Check" "Medium" "Unknown" "Install Az.Monitor and verify Azure Monitor permissions."
    }
    ...
    # Log the operation
    Write-MATLog -OperationName "AuditorMode" -Details "Generated Auditor_Report.csv with $($auditResults.Count) checks. Critical: $criticalCount, Warnings: $warningCount."
