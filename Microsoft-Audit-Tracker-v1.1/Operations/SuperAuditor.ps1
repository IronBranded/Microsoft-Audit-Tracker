...
function Invoke-SuperAuditor {
    ...
    Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Blue
    try {
        # Bug Fix #2: Pass -Silent so each mode skips its own Pause.
        # Without this, the user was prompted to press Enter 3 times before
        # the summary screen ever appeared.
        Invoke-AuditorMode -Silent
        $successCount++
...
function New-MATHtmlReport {
    ...
    # Bug Fix #10: Tenant names and user principals are user-supplied strings that can contain
    # HTML special characters (e.g. "Contoso & Sons <EU>"). Encode them before interpolation
    # so the report header doesn't break or expose an injection vector.
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value">$safeTenantHtml</div>
            </div>
            <div class="stat-card">
                <div class="stat-label"> Auditor Principal</div>
                <div class="stat-value" style="font-size: 18px;">$safeUserHtml</div>
            </div>
            <div class="stat-card">
