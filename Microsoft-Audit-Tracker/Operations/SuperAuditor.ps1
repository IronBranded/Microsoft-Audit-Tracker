...
function New-SuperAuditorHTMLReport {
    ...
    $auditorData = if (Test-Path $auditorFile) { Import-Csv $auditorFile } else { @() }
    $protectorData = if (Test-Path $protectorFile) { Import-Csv $protectorFile } else { @() }
    $defensiveData = if (Test-Path $defensiveFile) { Import-Csv $defensiveFile } else { @() }
    $licensesData = if (Test-Path $licensesFile) { Import-Csv $licensesFile } else { @() }
...
"@
        foreach ($row in $auditorData) {
            $statusClass = if ($row.Status -match "Enabled|Active|Success") { "status-enabled" } else { "status-disabled" }
...
"@
        foreach ($row in $protectorData) {
            $stateClass = if ($row.State -match "Enabled|Active") { "status-enabled" } else { "status-disabled" }
...
"@
        foreach ($row in $defensiveData) {
            $statusClass = if ($row.Status -match "Active") { "status-active" } else { "status-inactive" }
...
function Invoke-SuperAuditor {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "[+] Operation [4] successful. Inventory saved to $outFile" -ForegroundColor Green
...
function Show-MATMenu {
    ...
    Write-Host ""
        ...
        switch ($selection) {
            ...
            "D" { Invoke-Diagnostic }
            "Q" { exit }
            default { Write-Host "[!] Invalid Selection" -ForegroundColor Red; Start-Sleep -Seconds 4 }
        }
    }
}
